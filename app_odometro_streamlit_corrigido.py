...
# ============================================================
# V16 - ODOMETRO_MATCH UNICO + CORRECAO DECIMAL MAXTRACK + CLIENTE CHAVEADO
# ============================================================
# Regras principais:
...
def para_numero(valor):
    """
    Converte numeros em formatos BR e US sem destruir casas decimais.

    Correção importante V16:
    - A Maxtrack pode trazer odometro como 348272.8 usando ponto decimal.
    - A rotina anterior removia todo ponto e transformava 348272.8 em 3482728,
      gerando saltos artificiais e planilha incoerente.
    """
    if pd.isna(valor):
        return np.nan
    ...
    txt = str(valor).strip()
    if not txt or txt in {"-", "--", "nan", "NaN", "None"}:
        return np.nan

    # Limpa moeda, unidades e espaços mantendo sinais e separadores.
    txt = txt.replace("R$", "").replace("KM", "").replace("km", "").strip()
    txt = txt.replace(" ", "")

    try:
        # Formato com virgula e ponto: decide pelo ultimo separador.
        ...
        # Apenas virgula: trata como decimal BR.
        if "," in txt:
            return float(txt.replace(".", "").replace(",", "."))

        # Apenas ponto: preserva como decimal US/Excel.
        # Se houver muitos pontos, remove pontos de milhar e preserva o ultimo como decimal.
        if txt.count(".") > 1:
            partes = txt.split(".")
...
def ler_maxtrack_rapido_xlsx(caminho):
    # V16: nao usa cache antigo. Se algum cache foi criado com a conversao antiga,
    # ele pode conter odometro multiplicado por 10 e contaminar o resultado.
    ns = "{http://schemas.openxmlformats.org/spreadsheetml/2006/main}"
    colunas = {
...
def app_streamlit():
    st.set_page_config(
        page_title="ODOMETRO_MATCH V16 - Maxtrack + Cliente",
        page_icon="🚚",
        layout="wide",
    )
...
- AM/PM e usado apenas internamente, sem gerar coluna no Excel.
    ...
    nome_saida = st.text_input(
        ...
        value="Resumo_Abastecimento_Odometro_V16.xlsx",
    )
...
# ============================================================
# COMPATIBILIDADE COM PORTAIS / MULTIPAGE STREAMLIT
# ============================================================
# Alguns portais Streamlit importam o arquivo como modulo. Nessa situacao,
# o bloco __main__ nao executa e a tela fica em branco. Por isso mantemos
# aliases padrao e renderizacao automatica quando houver contexto Streamlit.

_APP_ODOMETRO_RENDERIZADO = False


def render():
    global _APP_ODOMETRO_RENDERIZADO
    if not _APP_ODOMETRO_RENDERIZADO:
        _APP_ODOMETRO_RENDERIZADO = True
        app_streamlit()
...
def _esta_rodando_em_streamlit():
    try:
...
if __name__ == "__main__":
    render()
elif _esta_rodando_em_streamlit():
    render()
