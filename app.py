...
def inspecionar_script(nome_arquivo: str) -> dict[str, Any]:
    ...
    info = {
        ...
        "usa_streamlit": False,
        "codigo_solto": False,
        "tem_set_page_config": False,
        "tem_input_console": False,
        "erro": "",
    }
    ...
    try:
        ...
        info["parse_ok"] = True
        info["usa_streamlit"] = bool(re.search(r"\bstreamlit\b|\bst\.", texto))
        info["tem_set_page_config"] = "set_page_config" in texto
        info["tem_input_console"] = bool(re.search(r"\binput\s*\(", texto))
        ...
        for node in tree.body:
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                info["codigo_solto"] = True
            elif isinstance(node, ast.Expr):
                # Docstring pura não conta como código solto. Outras expressões no topo podem executar lógica.
...
def recomendar_modo_execucao(info: dict[str, Any]) -> str:
    """Escolhe o modo mais seguro para executar um .py sem alterar a lógica original."""
    if info.get("main_streamlit"):
        return "main_streamlit"
    if info.get("main"):
        return "main"
    if info.get("usa_streamlit"):
        return "script"
    return "subprocess"


def executar_script_subprocess(caminho: Path) -> None:
    """Executa scripts que não são tela Streamlit sem travar a interface do portal."""
    st.info("Executando como processo isolado. Saídas aparecerão abaixo; interfaces Streamlit precisam usar main_streamlit() ou modo script.")
    log_box = st.empty()
    ...
    while True:
        ...
        if time.time() - inicio > 900:
            proc.kill()
            st.error("Execução interrompida por limite de 15 minutos.")
            return
        time.sleep(0.2)
    if proc.returncode == 0:
        st.success("Script finalizado com sucesso.")
    else:
        st.error(f"Script finalizado com erro. Código de saída: {proc.returncode}")
...
def executar_modulo_config(config: dict[str, Any]) -> None:
    ...
    if modo == "subprocess":
        executar_script_subprocess(caminho)
        return
    ...
    try:
        ...
        if modo == "main_streamlit":
            if not hasattr(modulo, "main_streamlit"):
                st.warning("main_streamlit() não foi encontrada. O portal executará como script integrado, sem alterar o arquivo original.")
                executar_script_integrado(caminho)
                return
            modulo.main_streamlit()
        elif modo == "main":
            if not hasattr(modulo, "main"):
                st.warning("main() não foi encontrada. O portal executará como script integrado, sem alterar o arquivo original.")
                executar_script_integrado(caminho)
                return
            modulo.main()
        else:
        ...
        if not scripts:
            ...
            if info["parse_ok"]:
                ...
                st.success(
                    ...
                    f"main: {info['main']} | usa_streamlit: {info.get('usa_streamlit')} | "
                ...
                render_notice(
                    ...
                    "Se o arquivo não for uma tela Streamlit, o modo subprocess é mais seguro para evitar travamentos.",
                    "info",
                )
            else:
            ...
            with col2:
                ...
                modo = st.selectbox("Modo de execução", ["auto", "main_streamlit", "main", "script", "subprocess"], index=["auto", "main_streamlit", "main", "script", "subprocess"].index(str(cfg.get("modo", "auto"))) if str(cfg.get("modo", "auto")) in ["auto", "main_streamlit", "main", "script", "subprocess"] else 0, key=f"editor_modo_{selecionado}")
...
def pagina_modulo_dinamico(config: dict[str, Any]) -> None:
    nome = str(config.get("nome", "Módulo"))
    arquivo = str(config.get("arquivo", ""))
    modo = str(config.get("modo", "auto"))
    render_page_head(nome, str(config.get("descricao", "Módulo adicional configurado.")))

    info = inspecionar_script(arquivo) if arquivo else {"parse_ok": False, "erro": "Arquivo não informado"}
    modo_real = recomendar_modo_execucao(info) if modo == "auto" and info.get("parse_ok") else modo

    st.markdown("### Preparação do módulo")
    st.write(f"**Arquivo:** `{arquivo}`")
    st.write(f"**Modo configurado:** `{modo}`  |  **Modo que será usado:** `{modo_real}`")

    if not info.get("parse_ok"):
        st.error(f"Não é possível abrir este módulo: {info.get('erro', '')}")
        return

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("main_streamlit", "Sim" if info.get("main_streamlit") else "Não")
    c2.metric("main", "Sim" if info.get("main") else "Não")
    c3.metric("usa Streamlit", "Sim" if info.get("usa_streamlit") else "Não")
    c4.metric("modo", modo_real)

    render_notice(
        "O portal não executa módulos adicionais automaticamente ao entrar na página. "
        "Isso evita travamento quando um script legado é pesado ou tem código solto. "
        "Clique em abrir para carregar a interface ou executar o processamento.",
        "info",
    )

    exec_key = "executar_modulo_" + slugify(arquivo)
    col_a, col_b = st.columns([1, 1])
    with col_a:
        if st.button("Abrir / executar módulo", use_container_width=True, type="primary", key="btn_run_" + exec_key):
            st.session_state[exec_key] = True
            st.rerun()
    with col_b:
        if st.button("Fechar módulo", use_container_width=True, key="btn_close_" + exec_key):
            st.session_state[exec_key] = False
            st.rerun()

    if st.session_state.get(exec_key, False):
        st.divider()
        st.markdown("### Execução do módulo")
