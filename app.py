    ...
    if pagina == nome_menu:
        ...
        try:
            ...
            if possui_main_streamlit or possui_main:
                ...
                with col1:
                    if st.button(
                        f"🚀 Executar {nome_amigavel_script(script)}",
                        use_container_width=True,
                        key=f"btn_executar_{script}"
                    ):
                        st.session_state.modulo_ativo = script

                with col2:
                    if st.button(
                        "🔄 Fechar módulo",
                        use_container_width=True,
                        key=f"btn_fechar_{script}"
                    ):
                        st.session_state.modulo_ativo = None
                        st.rerun()

                if st.session_state.modulo_ativo == script:
                    st.divider()
