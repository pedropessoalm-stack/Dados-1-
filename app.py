...
def pagina_odometro() -> None:
    ...
    render_message(
        "info",
        "Envie as quatro bases obrigatórias. O processamento será executado pelo módulo "
        "<b>app_odometro_streamlit_corrigido</b>, sem alterar a lógica interna enviada.",
    )

    modulo_odometro_path = encontrar_arquivo(
        [
            "app_odometro_streamlit_corrigido (1).py",
            "app_odometro_streamlit_corrigido.py",
            "Python_Odometro_Vinculo.py",
    ...
    if modulo_odometro_path is None:
        render_message(
            "error",
            "Módulo de odômetro não encontrado. Envie o arquivo "
            "<b>app_odometro_streamlit_corrigido.py</b> na mesma pasta do app.py.",
        )
        return
    ...
    nome_saida = st.text_input(
        ...
        value=f"resultado_odometro_v12_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
        key="odometro_nome_saida",
    )

    if st.button("Processar Odômetro V12", key="btn_processar_odometro_integrado", use_container_width=True, type="primary"):
        inicio = time.time()
        try:
            modulo_odometro = carregar_modulo_por_arquivo(modulo_odometro_path)

            if not hasattr(modulo_odometro, "processar_streamlit"):
                render_message(
                    "error",
                    "O módulo carregado não possui a função <code>processar_streamlit()</code>, "
                    "necessária para rodar integrado ao aplicativo.",
                )
                return

            with st.spinner("Processando bases e aplicando regras de ODOMETRO_MATCH..."):
                excel_bytes, indicadores, resultado_final = modulo_odometro.processar_streamlit(
                    comb,
                    maxtrack,
                    ativo,
                    producao,
            ...
            if resultado_final is not None:
                ...
                k1.metric("Registros finais", f"{len(resultado_final):,}".replace(",", "."))
                if "ODOMETRO_MATCH" in resultado_final.columns:
                    k2.metric("ODOMETRO_MATCH preenchido", f"{int(resultado_final['ODOMETRO_MATCH'].notna().sum()):,}".replace(",", "."))
                if "KM_ENTRE_ABAST" in resultado_final.columns:
                    k3.metric("Saltos > 1500", int((resultado_final["KM_ENTRE_ABAST"] > 1500).sum()))
                if "DIFERENCA_ODOM_X_CLIENTE" in resultado_final.columns:
                    abaixo_cliente = int(((resultado_final["DIFERENCA_ODOM_X_CLIENTE"] < -1e-9) & resultado_final["DIFERENCA_ODOM_X_CLIENTE"].notna()).sum())
                    k4.metric("Abaixo Cliente ajustado", abaixo_cliente)
            ...
            st.download_button(
                ...
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
                key="download_odometro_integrado",
            )
