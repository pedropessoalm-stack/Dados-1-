...
if LOGO_B64:
    BRAND_MARK = f'<img src="data:image/jpeg;base64,{LOGO_B64}" class="brand-img" alt="Expresso Nepomuceno">'
else:
...
def pagina_inicio(modulos: list[dict[str, Any]]) -> None:
            ...
            <div class="hero-content">
                ...
                <div class="module-glass">
                    <div class="module-grid">
                        <div class="module-item"><div class="module-icon">◷</div><div class="module-title">Permanência</div><div class="module-desc">Tempos, eventos e padrões operacionais</div></div>
                        <div class="module-item"><div class="module-icon">⏱</div><div class="module-title">Odômetro</div><div class="module-desc">Leituras, bases e consolidação V12</div></div>
                        <div class="module-item"><div class="module-icon">⚙</div><div class="module-title">Editor</div><div class="module-desc">Configure módulos sem alterar o código</div></div>
                        <div class="module-item"><div class="module-icon">⛽</div><div class="module-title">Combustível</div><div class="module-desc">Módulos adicionais configuráveis</div></div>
                        <div class="module-item"><div class="module-icon">▣</div><div class="module-title">Bases</div><div class="module-desc">Arquivos de entrada e processamento</div></div>
                        <div class="module-item"><div class="module-icon">▤</div><div class="module-title">Relatórios</div><div class="module-desc">Saídas tratadas em Excel</div></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    with c1:
        if st.button("Abrir Permanência", use_container_width=True, type="primary"):
            ir_para("permanencia")
    with c2:
        if st.button("Abrir Odômetro V12", use_container_width=True, type="primary"):
            ir_para("odometro")
    with c3:
        if st.button("Abrir Editor de Módulos", use_container_width=True):
            ir_para("editor")
...
def pagina_permanencia() -> None:
    ...
    message("success", f"Arquivo carregado: <b>{html.escape(arquivo.name)}</b>")
    ...
    if st.button("Processar Permanência", use_container_width=True, type="primary"):
        ...
        try:
            ...
            st.success(f"Processo finalizado em {round(time.time() - inicio, 2)} segundos")
...
def pagina_odometro() -> None:
    ...
    message("success", "Todas as bases foram carregadas. O processamento já pode ser iniciado.")
    ...
    if st.button("Processar Odômetro V12", use_container_width=True, type="primary"):
        ...
        try:
            ...
            atualizar_progresso(barra, status, 100, "Finalizado")
            st.success(f"Odômetro finalizado em {round(time.time() - inicio, 2)} segundos")
            with open(saida, "rb") as f:
                st.download_button(
