...
def pagina_inicio(scripts_detectados: list[str]) -> None:
    render_page_title(
        "🏠",
        "Visão geral",
        "Central de módulos para processamento e acompanhamento de dados operacionais.",
    )

    col1, col2, col3, col4 = st.columns(4)
    with col1:
        render_kpi("Módulos ativos", str(len(scripts_detectados) + 2))
    with col2:
        render_kpi("Layout", "Executivo")
    with col3:
        render_kpi("Scripts detectados", str(len(scripts_detectados)))
    with col4:
        render_kpi("Status", "Online")
    ...
    with c1:
        render_card(
            ...
            "Processa base de permanência, identifica eventos, classifica tempos e gera Excel final para acompanhamento operacional.",
        )
    with c2:
        render_card(
            "🚛 Odômetro V12",
            "Cruza bases de combustível, Maxtrack, ativos e produção oficial para consolidação do relatório final.",
        )
    with c3:
        render_card(
            "🧩 Módulos dinâmicos",
    ...
    if scripts_detectados:
        st.markdown("### Arquivos Python detectados")
        dados = [{"Arquivo": s, "Nome no menu": nome_amigavel_script(s)} for s in scripts_detectados]
        st.dataframe(dados, use_container_width=True, hide_index=True)

    render_message(
        "info",
        "Para adicionar novos módulos, coloque o arquivo <b>.py</b> na mesma pasta do <b>app.py</b> e inclua uma função <code>main_streamlit()</code> ou <code>main()</code>.",
    )
