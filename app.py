...
def get_module_diagnostics() -> list[DiagnosticItem]:
    ...
    od_file = find_file(["Python_Odometro_Vinculo.py"])
    if od_file and od_file.exists():
        try:
            ...
            if missing:
                ...
                items.append(DiagnosticItem("Odômetro / Vínculo", od_file.name, "Cruzar combustível, maxtrack, ativos e produção", "OK", "Estrutura principal encontrada."))
        except Exception as exc:
            items.append(DiagnosticItem("Odômetro / Vínculo", od_file.name, "Cruzar combustível, maxtrack, ativos e produção", "Erro", str(exc)))
    else:
        items.append(DiagnosticItem("Odômetro / Vínculo", "Python_Odometro_Vinculo.py", "Cruzar combustível, maxtrack, ativos e produção", "Erro", "Arquivo não localizado."))
...
def page_odometro() -> None:
    ...
    module_path = find_file(["Python_Odometro_Vinculo.py"])
    if not module_path:
        st.error("Arquivo Python_Odometro_Vinculo.py não encontrado.")
        return
