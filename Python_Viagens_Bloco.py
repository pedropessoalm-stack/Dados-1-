...
def processar_arquivos(caminho_maxtrack, caminho_sap, caminho_perm, progress=None):
    ...
    parametros = pd.DataFrame({
        "BASE": [
            "MAXTRACK", "SAP", "PERMANENCIA", "PERMANENCIA", "PERMANENCIA", "PERMANENCIA", "PERMANENCIA", "TRAVA", "DEDUP"
        ],
        "USO": [
...
def _salvar_upload_streamlit(uploaded_file, prefixo):
    import tempfile
    from pathlib import Path
    sufixo = Path(uploaded_file.name).suffix or ".xlsx"
    with tempfile.NamedTemporaryFile(delete=False, suffix=sufixo, prefix=prefixo) as tmp:
        tmp.write(uploaded_file.read())
        return tmp.name


def main_streamlit():
    """Interface Streamlit sem tkinter para executar a lógica original."""
    import streamlit as st
    st.subheader("Viagens em Bloco")
    st.caption("Processamento integrado de Maxtrack, SAP e Permanência em Área com validação, score e deduplicação.")

    c1, c2, c3 = st.columns(3)
    with c1:
        arquivo_max = st.file_uploader("1. Maxtrack", type=["xlsx", "xls"], key="vb_maxtrack_modulo")
    with c2:
        arquivo_sap = st.file_uploader("2. SAP", type=["xlsx", "xls"], key="vb_sap_modulo")
    with c3:
        arquivo_perm = st.file_uploader("3. Permanência em Área", type=["xlsx", "xls"], key="vb_perm_modulo")

    pronto = all([arquivo_max, arquivo_sap, arquivo_perm])
    if not pronto:
        st.warning("Envie os três arquivos obrigatórios para liberar o processamento.")
        return

    if st.button("Processar Viagens em Bloco", type="primary", use_container_width=True, key="vb_processar_modulo"):
        progress = StreamlitProgressAdapter()
        try:
            p_max = _salvar_upload_streamlit(arquivo_max, "maxtrack_")
            p_sap = _salvar_upload_streamlit(arquivo_sap, "sap_")
            p_perm = _salvar_upload_streamlit(arquivo_perm, "permanencia_")
            saida, total_viagens, total_placas, total_invalidas, total_duplicadas, total_antes_dedup = processar_arquivos(
                p_max, p_sap, p_perm, progress
            )
            st.success("Processamento concluído com sucesso.")
            c1, c2, c3, c4, c5 = st.columns(5)
            c1.metric("Antes da dedup", total_antes_dedup)
            ...
            st.error(f"Erro ao processar Viagens em Bloco: {exc}")
