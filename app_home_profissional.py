...
/* Navegação interna: garante saída mesmo quando a sidebar estiver oculta/recolhida */
.co-page-nav-shell {{
...
/* Correção extra de contraste dos rótulos nativos do Streamlit */
...
def render_page_nav() -> None:
    """Navegação interna para não depender apenas da sidebar."""
    st.markdown('<div class="co-page-nav-shell"><div class="co-page-nav-title">Navegação rápida</div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        if st.button("🏠 Início", key="topnav_inicio", use_container_width=True):
            ir_para("inicio")
    with c2:
        if st.button("📊 Permanência", key="topnav_permanencia", use_container_width=True):
            ir_para("permanencia")
    with c3:
        if st.button("🚛 Odômetro", key="topnav_odometro", use_container_width=True):
            ir_para("odometro")
    with c4:
        if st.button("🛠️ Editor", key="topnav_editor", use_container_width=True):
            ir_para("editor_modulos")
